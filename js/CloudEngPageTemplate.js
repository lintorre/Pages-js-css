...
$(document).ready(function () {
    ...
    setTimeout($(document).ready(function () {
        $("body").find(".CenterNarrow").parents(".innerCell").addClass("main-table-content");
        /*
    ...
    if ($('body h1').length > 0) {
        $('#blogContents').prepend('<nav class="toc"><h3 class="title" style="padding: 4% 2% 0% 4%;">Table of Contents</h3><ul></ul></nav>');
    }

    $('body').find("h1:not(#title-text,#logo)").each(function () {
        var $item = $(this);
        var $id = $(this).attr('id');
        var li = $('<li/>');
...
$(function () {
    ...
    function myFunction() {
        //var thisScrollTop = ("THIS  " + ($(this).scrollTop().toFixed(2)) + " <br>  lightSlider  <br> " + $("#lightSlider").offset().top.toFixed(2)) ;
        /* if (window.pageYOffset > $("#CaseStudies").offset().top) {
                 header.classList.remove("sticky");
                 $('#myHeader').css({position: 'sticky', top: 22+"%"});              }            */
        if (window.pageYOffset > sticky) {
            header.classList.add("sticky");
            $('#myHeader').css({ position: 'fixed', top: 22 + "%" });
        }/*
        else {
            header.classList.remove("sticky");
            $('#myHeader').css({ position: 'sticky', top: 22 + "%" });
